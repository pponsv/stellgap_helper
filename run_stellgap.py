...
def parse_args():
    ...
    parser.add_argument(
        "--ext", metavar="Extension", help="Extension name of the VMEC file"
    )
    parser.add_argument(
        "--sound",
    ...
    parser.add_argument("--vmec", help="Run VMEC on input.ext", action="store_true")
    parser.add_argument("--booz", help="Run BOOZ_XFORM", action="store_true")
    parser.add_argument("--xst", help="Run STELLGAP", action="store_true")
    parser.add_argument("--xmetric", help="Run XMETRIC", action="store_true")
    parser.add_argument(
        "--fine",
        metavar="ir_fine",
...
def call_xstgap(dirname, irads, ir_fine_scl, sound=False, logname="log.tmp", rw=False):
    ...
    result = subprocess.Popen(
        [executable, str(irads), str(ir_fine_scl)], stdout=subprocess.PIPE
    )
    with open(logname, "wb") as out:
...
def run_all(
    ...
    if XSTGAP:
        print("\nCall xstgap\n")
        result_xstgap = call_xstgap(
            dirname, irads=(nsurf - 2), ir_fine_scl=num_fine, sound=SOUND, rw=RW
        )
        print(result_xstgap.args)
